---
title: "MAST Document"
source_url: "https://outerspace.stsci.edu/spaces/MASTDOCS/pages/117833935/Data+Product+File+Formats"
date_accessed: "2026-03-11T11:33:17.261659"
---

<!-- CHUNK 1 START -->
<!-- Metadata: {"Header 1": "File Format Summary"} -->
Most of the JWST science data files are images or tables in
Product type | Format | Description
---|---|---
|  | Science data products for imaging, spectroscopy, coronagraphy, and (early stages of) time-series observations. The image extensions contain multi-dimensional science pixel arrays, concomitant data quality flags, and variance arrays. See the article on
| Extracted spectra are stored as tables, and include fields for spectral coordinates, flux, and concomitant quality information. [AMI](https://jwst-docs.stsci.edu/jwst-near-infrared-imager-and-slitless-spectrograph/niriss-observing-modes/niriss-aperture-masking-interferometry) fringe parameters are also stored as binary tables of coefficients.
| Source catalogs, as well as time-series observation ([TSO](https://jwst-docs.stsci.edu/methods-and-roadmaps/jwst-time-series-observations)) light curves are stored in an ASCII _enhanced character-separated variable_ format.
| Metadata for images or spectra are stored in ASDF format, but are packaged as BYTE arrays in a FITS extension of type BINTABLE.
|  | Data products from associated guide-star activities: identification, acquisition, tracking and guiding. The image extensions are analogous to those for science data.
|  | A collection of metadata for all exposures that match a combination of attributes, including instrument, program ID, and observation, from which associations are generated within a given observing program.
| Enumeration of relationships between different levels of science data products, and related calibration reference products, that were used during the course of calibration pipeline processing.
|  | Wavefront-sensing Optical Path Difference (OPD) images that describe the state of the mirror segment alignments for a particular epoch. The OPD files can be used with a tool such as
...
|  | Static reference files used in calibration processing come in various formats.
